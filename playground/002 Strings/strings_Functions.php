<html>
<head>
	<meta charset="UTF-8">
	<title>Strings</title>
</head>
<body>

<?php 

	$first = "The quick brown fox";
	$second = " jumped over the lazy dog.";

	# Another way for doing concatenation

	$third = $first;
	$third .= $second;

	# Third is concatednated to second

	echo $third;

 ?>

 <br />



<?php 
# php calling some functions

 // strtolower the name of the Function. Function takes something as an argument
 // the input will be $third
?>

Lowercase: <?php echo strtolower($third); ?><br />
Uppercase: <?php echo strtoupper($third); ?><br />
Uppercase first: <?php echo ucfirst($third); ?><br />
Uppercase words: <?php echo ucwords($third); ?><br />
<br />

Length: <?php echo strlen($third); ?><br />
Trim: <?php echo "A" . trim(" B C D ") . "E"; ?><br />
Find: <?php echo strstr($third, "brown"); ?>
Replace by string: <?php echo str_replace("quick", "super-fast", $third); ?><br />

<br />
Repeat: <?php  echo str_repeat($third, 2); ?><br />
Make substring: <?php echo substr($third, 5, 10); ?><br />
Find position: <?php echo strpos($third, "brown") ?><br />
Find character: <?php echo strchr($third, "z") ?><br />



	
</body>
</html>